...
class AdapterKomunitas(private val listDataku: ArrayList<DataClassViewKomun>): RecyclerView.Adapter<AdapterKomunitas.FirebaseViewHolder>() {
    inner class FirebaseViewHolder(myView: View): RecyclerView.ViewHolder(myView) {
        var tv_Nomor: TextView = myView.findViewById(R.id.tv_noKomun)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): FirebaseViewHolder {
        val viewku: View = LayoutInflater.from(parent.context).inflate(R.layout.view_data_komunitas, parent, false)
    ...
    override fun onBindViewHolder(holder: FirebaseViewHolder, position: Int) {
        ...
        holder.btnBk.setOnClickListener{
            val intent = Intent(holder.itemView.context, DataPelajar::class.java)
            val bundle = Bundle()
